//! ng-repeat/ng-init式から変数をパースする共通モジュール

use crate::index::HtmlLocalVariableSource;
...
/// ng-repeat式から変数をパース
/// 例: "item in items" -> [ParsedVariable { name: "item", ... }]
/// 例: "(key, value) in obj" -> [ParsedVariable { name: "key", ... }, ParsedVariable { name: "value", ... }]
...
/// ng-init式から変数をパース
/// 例: "a = 1" -> [ParsedVariable { name: "a", ... }]
/// 例: "a = 1; b = 2" -> [ParsedVariable { name: "a", ... }, ParsedVariable { name: "b", ... }]
...
mod tests {
    ...
    fn test_parse_ng_repeat_simple() {
        ...
        assert!(matches!(vars[0].source, HtmlLocalVariableSource::NgRepeatIterator));
    }
    ...
    fn test_parse_ng_repeat_key_value() {
        ...
        assert!(matches!(vars[0].source, HtmlLocalVariableSource::NgRepeatKeyValue));
        assert!(matches!(vars[1].source, HtmlLocalVariableSource::NgRepeatKeyValue));
    }
