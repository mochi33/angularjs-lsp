...
use crate::model::{
    ComponentTemplateUrl, ExportInfo, ExportedComponentObject, Span, SymbolBuilder, SymbolKind,
    SymbolReference,
};
...
impl AngularJsAnalyzer {
                ...
                if !service_name.starts_with('$') {
                    ...
                    let reference = SymbolReference {
                        ...
                        span: Span::new(
                            ...
                            if end.column > 0 { end.column as u32 - 1 } else { end.column as u32 },
                        ),
                    };
    ...
    fn collect_local_vars_for_export(
        ...
        node: Node,
        source: &str,
        local_vars: &mut std::collections::HashMap<String, Span>,
    ) {
        match node.kind() {
    ...
    fn scan_all_patterns(
        ...
        component_name: &str,
        this_aliases: &[String],
        local_vars: &std::collections::HashMap<String, Span>,
    ) {
        match node.kind() {
                        ...
                        if let Some(value) = child.child_by_field_name("value") {
                            ...
                            match value.kind() {
                                ...
                                "identifier" => {
                                    ...
                                    if let Some(var_span) = local_vars.get(&var_name) {
                                        let docs =
                                            self.extract_jsdoc_for_line(var_span.start_line as usize, source);

                                        let mut builder = SymbolBuilder::new(full_name, SymbolKind::Method, uri.clone())
                                            .definition_span(*var_span)
                                            .name_span(name_span);
