...
impl CodeLensHandler {
    ...
    fn code_lens_for_html(&self, uri: &Url) -> Option<Vec<CodeLens>> {
        ...
        let parents: Vec<_> = self
            .index
            .templates
            .get_parent_templates_for_child(uri)
            .into_iter()
        ...
        let bindings = self.index.templates.get_all_template_binding_sources(uri);
        ...
        let inherited = self
            .index
            .templates
            .get_inherited_controllers_for_template(uri);
        for controller in inherited {
            lenses.push(self.create_controller_lens(&controller, 0, "Inherited"));
        }

        // 5. HTML内のng-controllerスコープ
        let html_scopes = self.index.controllers.get_all_html_controller_scopes(uri);
        ...
        let ng_includes = self.index.templates.get_ng_includes_in_file(uri);
        ...
        for symbol in symbols {
            if symbol.kind == SymbolKind::Controller {
                ...
                if !templates.is_empty() {
                    lenses.push(self.create_template_lens(
                        &symbol.name,
        ...
        let bindings = self.index.templates.get_template_bindings_for_js_file(uri);
    ...
    fn code_lens_for_js(&self, uri: &Url) -> Option<Vec<CodeLens>> {
        ...
        let template_urls = self.index.components.get_component_template_urls(uri);
            ...
            let locations: Vec<serde_json::Value> = definitions
                ...
                .map(|def| {
                    serde_json::json!({
                        ...
                        "range": {
                            "start": { "line": def.definition_span.start_line, "character": def.definition_span.start_col },
                            "end": { "line": def.definition_span.start_line, "character": def.definition_span.start_col }
                        }
                    })
                })
                .collect();
    ...
    fn create_binding_lens(&self, binding: &TemplateBinding) -> CodeLens {
        ...
        let title = format!(
            "{}: {} -> {}",
            source_label, template_filename, binding.controller_name
        );
        ...
        let definitions = self
            .index
            .definitions
            .get_definitions(&binding.controller_name);
